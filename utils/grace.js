...
module.exports = {
	...
	navigate:function (url, type, success, fail, complete) {
		...
		switch(type){
			case 'navigateTo' : 
			wx.navigateTo({url:url, success:success, fail:fail, complete:complete});
			break;
			case 'redirectTo' : 
			wx.redirectTo({url:url, success:success, fail:fail, complete:complete});
			break;
			case 'switchTab' : 
			wx.switchTab({url:url, success:success, fail:fail, complete:complete});
			break;
			case 'reLaunch' : 
			wx.reLaunch({url:url, success:success, fail:fail, complete:complete});
			break;
		}
	},
	// 返回
	...
	setStorage : function(data){
		try {
			for(let k in data){wx.setStorageSync(k, data[k]+'');}
			return true;
	...
	setNavBar : function(sets){
		if(sets.title){wx.setNavigationBarTitle({title:sets.title});}
		if(sets.color){
	...
	selectAll : function (selector, callBack) {
		wx.createSelectorQuery().selectAll(selector).boundingClientRect().exec((res)=>{callBack(res[0]);});
	},
